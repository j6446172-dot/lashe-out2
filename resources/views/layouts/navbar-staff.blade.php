    ...
    @if(auth()->user()->role === 'staff')
        ...
        <script>
            ...
            function openChatWindow() { 
                ...
                fetch('{{ route("staff.chat.mark-read") }}', {
                    method: 'POST',
                    headers: {
            ...
            async function loadChatMessages() { 
                try { 
                    const r = await fetch('{{ route("staff.chat.messages") }}'); 
                    const msgs = await r.json(); 
                    const c = document.getElementById('chatMessages'); 
                    if(msgs.length > 0) { 
                        let nid = msgs[msgs.length - 1].id; 
                        if(nid > lastMessageId) { 
                            c.innerHTML = ''; 
                            msgs.forEach(m => addMessageToPopup(m)); 
                            lastMessageId = nid; 
                            c.scrollTop = c.scrollHeight; 
                        } 
                    } else if(c.innerHTML === '' || c.innerHTML.includes('ابدأ المحادثة')) {
                        c.innerHTML = '<div class="text-center text-gray-400 text-sm py-10"><i class="fas fa-comments text-4xl mb-2 block"></i><p>ابدأ المحادثة مع المالك</p></div>';
                    }
                } catch(e) { 
                    console.error(e); 
                } 
            ...
            function addMessageToPopup(m) { 
                const c = document.getElementById('chatMessages'); 
                const isMine = m.from_user_id == {{ auth()->id() }}; 
                if(c.innerHTML.includes('ابدأ المحادثة')) c.innerHTML = ''; 
                const d = document.createElement('div'); 
                if(isMine) { 
                    d.className = 'flex justify-end mb-2'; 
                    d.innerHTML = `<div class="max-w-[80%] bg-[#B08D57] text-white rounded-2xl rounded-br-none px-3 py-2 shadow"><p class="text-sm">${escapeHtml(m.message)}</p><p class="text-[10px] text-white/60 mt-1">${formatTime(m.created_at)}</p></div>`; 
                } else { 
                    d.className = 'flex justify-start mb-2'; 
                    d.innerHTML = `<div class="max-w-[80%] bg-white rounded-2xl rounded-bl-none px-3 py-2 shadow" style="border-right:2px solid #B08D57;"><div class="flex items-center gap-1 mb-1"><i class="fas fa-crown text-[#B08D57] text-[10px]"></i><span class="text-[10px] font-bold" style="color:#B08D57;">المالك</span></div><p class="text-sm text-gray-700">${escapeHtml(m.message)}</p><p class="text-[10px] text-gray-400 mt-1">${formatTime(m.created_at)}</p></div>`; 
                } 
                c.appendChild(d); 
            }
            
            // إضافة حدث إرسال الرسالة
            document.getElementById('popupChatForm')?.addEventListener('submit', async(e) => { 
                e.preventDefault(); 
                const i = document.getElementById('popupMessageInput'); 
                const msg = i.value.trim(); 
                if(!msg) return; 
                const btn = e.target.querySelector('button[type="submit"]'); 
                btn.disabled = true; 
                try { 
                    const r = await fetch('{{ route("staff.chat.send") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            function closeLeaveHistoryModal() { 
                const m = document.getElementById('leaveHistoryModal'); 
