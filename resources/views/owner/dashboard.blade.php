    ...
    <div style="flex: 1; padding: 25px; margin-left: 240px;">
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            ...
            <a href="{{ route('owner.staff') }}" 
               onclick="var b = this.querySelector('.notification-badge'); if(b) b.style.display='none';"
               class="rounded-xl p-5 text-center transition hover:-translate-y-1 hover:shadow-lg relative"
               style="text-decoration: none; background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(8px); border: 1px solid rgba(176, 141, 87, 0.15);">
                
                @if(($unreadStaffMessages ?? 0) > 0)
                    <span class="notification-badge" style="position:absolute;top:10px;right:10px;background:#ef4444;color:white;min-width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:bold;padding:0 6px;box-shadow:0 2px 8px rgba(0,0,0,0.2);z-index:10;">
                        {{ $unreadStaffMessages }}
                    </span>
                @endif

                <div class="text-4xl mb-2">👩‍💼</div>
                <p class="font-bold text-lg" style="color: #B08D57;">الموظفات</p>
                <p class="text-xs mt-1" style="color: #7C8574;">إدارة الفريق</p>
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    if (localStorage.getItem('chatOpened_any') === 'true') {
